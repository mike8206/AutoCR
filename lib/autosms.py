...
def main(url_dict, cr_id_path, phone_path, google_secret_path, google_token_path, chrome_driver_path):
    # read portal credential from txt file
    cridpw = {}
    try:
        with open(cr_id_path, encoding="ANSI") as f:
    ...
    try:
        with open(phone_path, encoding="ANSI") as f:
            phone_list = f.read()
    except:
        raise ValueError('簡訊檔案錯誤!!')

    # get event from google calendar
    SMSstring = google_calendar(google_secret_path, google_token_path)
    # chrome driver
