...
def background_sign(driver_ie, url_dict, session_id, vsidpw):
    ...
    # input the pin
    wait_page_load(driver_ie) 
    driver_ie.find_element_by_css_selector(url_dict['digiback_pin']).send_keys(vsidpw['pin'])
    
    # click the signing button
    wait_page_load(driver_ie) 
    driver_ie.find_element_by_css_selector(url_dict['digiback_btn']).click()
    ...
    try:
        ...
        DDL_reason.click()
        sleep(0.5)
        DDL_confirm = driver_ie.find_element_by_css_selector(url_dict['digiback_ddl_confirm'])
